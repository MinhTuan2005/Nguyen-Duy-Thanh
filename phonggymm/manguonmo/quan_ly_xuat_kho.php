...
//			var id = $("#mnv").val();
			...
			if( name == "" && tt == "")
				...
				$.get("tim_kiem_xuat_nhap.php",{hoten:name,tt:tt},function(data){
				$("#tb").html(data);
			});
			}
		});
...
//			$page = $_GET['page'];
			...
			$sql = "SELECT phieuxuat.*,user_noibo.id AS usid,user_noibo.name AS usname FROM phieuxuat,user_noibo WHERE phieuxuat.id_user_noibo = user_noibo.id LIMIT $colum_item OFFSET $offset";
			$sql1 = "SELECT phieuxuat.*,user_noibo.id AS usid,user_noibo.name AS usname FROM phieuxuat,user_noibo WHERE phieuxuat.id_user_noibo = user_noibo.id";
...
//		}
	?>
	<h1 align="center">Xuất hàng</h1>
	<br>
<!--	Tìm kiếm-->

	<div class="thanhngang">
		<div class="ngang1"><a href="add_phieu_xuat.php"><button>Tạo phiếu xuất</button></a></div>
		<div class="ngang2">
	...
	<table id="tb" class="table-hover table-bordered">
		...
		<tbody>
			...
			<tr align="center">
				...
				<td align="center">
					<a href="chitietxuathang.php?id=<?=$value['id']?>" name="in"><button style="width: 50px;">In</button></a>
		...
		<div class="pagination" style="justify-content: center">
			...
			<?php
				if($page > 3)
					{
			?>	
						<a class="page-item page-link" href="#" onClick='ajShow_qlxk(1);'>First</a>
			<?php
				}
			?>
				<?php
				...
				<?php
						...
						if($page != $i)
						{
							...
							{
				?>
							 <a class="page-item page-link" href="#" onClick='ajShow_qlxk(<?=$i?>);'><?=$i?></a>
				<?php
							}
						}
						else
				...
				?>
						...
						<a class="page-item page-link" href="#" onClick='ajShow_qlxk(<?=$sum_page?>);'>Last</a>
					<?php
				}
					?>
		</div>
