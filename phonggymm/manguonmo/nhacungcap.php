		...
		$("#timkiem").click(function(){
			var ht = $("#ht").val();
				if(ht == "")
					...
					{
						$.get("tim_kiem_ncc.php",{name:ht},function(data){
							$("#tb").html(data);
						});
					}
			});
...
//			$page = $_GET['page'];
			...
			$sql = "SELECT * FROM ncc LIMIT $colum_item OFFSET $offset";
...
//	Truy vấn nhóm
	...
	?>
	<h1 align="center">Thông tin nhà cung cấp</h1>
	<br>
<!--	Tìm kiếm-->
	<div class="thanhngang">
		<div class="ngang1"><a href="them_ncc.php"><button>Thêm nhà cung cấp</button></a></div>
		<div class="ngang2">
	...
	<table class="table-hover table-bordered" id="tb">
		...
		<tbody align="center">
			...
			<tr>
				...
				<td><?=$value['sdt']?></td>
				<td><?=$value['email']?></td>
				<td><a href="edit_ncc.php?id=<?=$value['id']?>"><button style="width: 50px;">Sửa</button></a>
		...
		<div class="pagination" style="justify-content: center">
			...
			<?php
				if($page > 3)
					{
			?>	
						<a class="page-item page-link" href="#" onClick='ajShow_ncc(1);'>First</a>
			<?php
				}
			?>
				<?php
				...
				<?php
						...
						if($page != $i)
						{
							...
							{
				?>
							 <a class="page-item page-link" href="#" onClick='ajShow_ncc(<?=$i?>);'><?=$i?></a>
				<?php
							}
						}
						else
				...
				?>
						...
						<a class="page-item page-link" href="#" onClick='ajShow_ncc(<?=$sum_page?>);'>Last</a>
					<?php
				}
					?>
		</div>
