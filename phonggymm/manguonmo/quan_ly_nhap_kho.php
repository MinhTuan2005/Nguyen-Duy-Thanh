...
//			var id = $("#mnv").val();
			...
			if( name == "" && tt == "")
				...
				$.get("tim_kiem_nhap_hang.php",{hoten:name,tt:tt},function(data){
				$("#tb").html(data);
			});
			}
		});
...
//			$page = $_GET['page'];
			...
			$sql = "SELECT phieunhap.*, ncc.name AS ten_ncc, user_noibo.name AS ten_nv, ncc.sdt FROM phieunhap,ncc,user_noibo WHERE phieunhap.id_ncc = ncc.id AND phieunhap.id_user_noibo = user_noibo.id LIMIT $colum_item OFFSET $offset";
			$sql1 = "SELECT phieunhap.*, ncc.name, user_noibo.name, ncc.sdt FROM phieunhap,chitietphieunhap,ncc,user_noibo WHERE phieunhap.id_ncc = ncc.id AND phieunhap.id_user_noibo = user_noibo.id AND phieunhap.id = chitietphieunhap.id_phieunhap";
...
//		}
	?>
	<h1 align="center">Nhập hàng</h1>
	<br>
<!--	Tìm kiếm-->

	<div class="thanhngang">
		<div class="ngang1"><a href="add_don_nhap.php"><button>Tạo đơn nhập</button></a></div>
		<div class="ngang2">
		...
		<tbody>
			...
			<tr align="center">
				...
				<td align="center">
					...
					<a href="Xoa.php?id=<?=$value["id"]?>&dc=phieunhap"><button style="width: 50px;" onClick="return confirm('Bạn có chắc xóa không?')">Xóa</button></a>
				</td>
			</tr>
			<?php
				}
		...
		<div class="pagination" style="justify-content: center">
			...
			<?php
				if($page > 3)
					{
			?>	
						<a class="page-item page-link" href="#" onClick='ajShow_qlnk(1);'>First</a>
			<?php
				}
			?>
				<?php
				...
				<?php
						...
						if($page != $i)
						{
							...
							{
				?>
							 <a class="page-item page-link" href="#" onClick='ajShow_qlnk(<?=$i?>);'><?=$i?></a>
				<?php
							}
						}
						else
				...
				?>
						...
						<a class="page-item page-link" href="#" onClick='ajShow_qlnk(<?=$sum_page?>);'>Last</a>
					<?php
				}
					?>
		</div>
